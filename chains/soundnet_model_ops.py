...
def conv2d(input, depth_in, depth_out, F_h=1, F_w=1, S_h=1, S_w=1, P_h=0, P_w=0, pad='VALID', name_scope='conv'):
    ...
    with tf.variable_scope(name_scope) as scope:
        ...
        try:
            w_conv = tf.get_variable('weights', [F_h, F_w, depth_in, depth_out], 
                    initializer=tf.truncated_normal_initializer(0.0, stddev=0.01))
        except ValueError:
            scope.reuse_variables()
            w_conv = tf.get_variable('weights')

        b_conv = tf.get_variable('biases', [depth_out], 
...
def batch_norm(input, depth_out, eps, name_scope='conv'):
    ...
    with tf.variable_scope(name_scope) as scope:
        ...
        try:
            mu_conv = tf.get_variable('mean', [depth_out], 
                initializer=tf.constant_initializer(0))
        except ValueError:
            scope.reuse_variables()
            mu_conv = tf.get_variable('mean')

        var_conv = tf.get_variable('var', [depth_out], 
            initializer=tf.constant_initializer(1))
        gamma_conv = tf.get_variable('gamma', [depth_out], 
            initializer=tf.constant_initializer(1))
        beta_conv = tf.get_variable('beta', [depth_out], 
            initializer=tf.constant_initializer(0))
        output = tf.nn.batch_normalization(input, mu_conv, 
...
def relu(input, name_scope='conv'):
    with tf.variable_scope(name_scope) as scope:
        return tf.nn.relu(input, name='a')
